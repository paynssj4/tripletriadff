...
from PyQt5.QtCore import Qt, QTimer, pyqtSignal, QObject  # Importer pyqtSignal et QObject

from card import Card

logging.basicConfig(level=logging.INFO)

class TripleTriadGame(QWidget):  # Hériter de QWidget    
    cardPlaced = pyqtSignal(int, Card)  # Signal pour indiquer qu'une carte a été placée
    ...
    def load_coordinates(self):
        try:
            with open("/home/greg/Documents/test jeux/test jeux tt/Nouveau dossier/fichier ancien/coordinates.json", 'r') as f:
    ...
    def display_player_cards(self):
        ...
        for card in self.player_cards:
            try:
                ...
                card_label = QLabel(f"Image not found for {card.name}", self.player_frame)
    ...
    def place_player_card(self, row, col):
        if self.selected_card and self.current_turn == "player":
            ...
            if self.game_board[index] is None:
                ...
                self.update_turn_indicator()
                self.cardPlaced.emit(index, self.game_board[index])  # Émettre le signal cardPlaced

                if self.ai_player:  # Vérifier si c'est un jeu contre l'IA
                    QTimer.singleShot(500, self.ai_turn)  # Attendre 5 secondes avant que l'IA ne joue

    def ai_turn(self):
        if self.current_turn == "opponent" and self.ai_player:  # Vérifier si c'est le tour de l'IA
            ...
            self.update_turn_indicator()
            self.cardPlaced.emit(position, self.game_board[position])
