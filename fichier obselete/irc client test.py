...
from PyQt5.QtGui import QPixmap, QImage, QPainter  # Importer QPainter ici
import irc.client
import logging
import threading
import json
...
from triple_triad_game2 import TripleTriadGame

logging.basicConfig(level=logging.INFO)

USERS_FILE = 'users.json'
CARDS_FILE = '/home/greg/Documents/test jeux/test jeux tt/Nouveau dossier/fichier ancien/cards.json'
...
class IRCClient(QWidget):
    ...
    def initialize_inventory_tab(self):
        ...
        self.selected_cards_area = QLabel(self.inventory_tab)  
        ...
        save_button = QPushButton("Sauvegarder", self.inventory_tab)
        save_button.clicked.connect(self.save_selection)
        layout.addWidget(save_button)

        # Bouton pour effacer la sélection
        clear_button = QPushButton("Effacer", self.inventory_tab)
        clear_button.clicked.connect(self.clear_selection)
        layout.addWidget(clear_button)

        self.load_selected_cards()  # Charger la sélection au démarrage

    def display_inventory(self):
        logging.debug("Displaying inventory...")
        ...
        for card in self.deck:
            ...
            try:
                ...
                if image.isNull():
                    raise FileNotFoundError(f"Image file not found: {card.image}")
                # Convertir QImage en QPixmap
                img = QPixmap.fromImage(image).scaled(50, 50, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        ...
        logging.debug("Inventory displayed.")
    ...
    def display_selected_cards(self):
        # Créer une image combinée des cartes sélectionnées
        combined_width = len(self.selected_cards) * 60  # Ajuster la largeur combinée en fonction du nombre de cartes
        ...
        for card in self.selected_cards:
            try:
                ...
                x_offset += img.width() + 10  # Ajouter un espacement entre les cartes
            except KeyError:
                pass
        painter.end()

        # Afficher l'image combinée dans le QLabel
    ...
    def clear_selection(self):
        for card_var in self.card_vars.values():
            card_var.setChecked(False)
        self.selected_cards = []
        self.display_selected_cards()  # Mettre à jour l'affichage

    def start_solo_game(self):
        logging.debug("Starting solo game...")
        if os.path.exists('selected_cards.json'):
            with open('selected_cards.json', 'r') as f:
                selected_cards_data = json.load(f)
            ...
            logging.debug(f"Player cards: {player_cards}")
            logging.debug(f"AI deck: {ai_deck}")

            self.game = TripleTriadGame(player_cards, ai_deck)  # Assurez-vous que self.game est une instance de TripleTriadGame
            logging.debug("TripleTriadGame instance created.")
            self.game.show()
            logging.debug("TripleTriadGame window shown.")
        else:
    ...
    def accept_defy(self, opponent, opponent_cards):
        if os.path.exists('selected_cards.json'):
            ...
            self.game = TripleTriadGame(player_cards, opponent_cards)
            self.game.show()
        else:
