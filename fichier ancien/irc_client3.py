...
# Importer la classe TripleTriadGame
from triple_triad_game_pygame import TripleTriadGame
...
def create_initial_deck(cards_file: str, deck_size: int = 40):
    ...
    all_cards = [Card(card_data["name"], card_data["attribute"],
                      card_data["numbers"], card_data["image"]) for card_data in all_cards_data]
...
class ScrolledFrame(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        # Définir des dimensions spécifiques pour le Canvas
        ...
        # Ne pas utiliser "ew" pour éviter de remplir horizontalement
...
class IRCClient:
    def __init__(self, root):
        ...
        self.nickname = simpledialog.askstring(
            "Create IRC Nickname", "Enter your IRC nickname:")
    ...
    def authenticate_user(self):
        ...
        response = messagebox.askyesno(
            "Authentication", "Do you have an account?")
        if response:
            ...
            if username in users and users[username]['password'] == password:
                ...
                self.deck = [Card(**card_data)
                                  for card_data in users[username]['deck']]
            ...
            username = simpledialog.askstring(
                "Create Account", "Enter your username:")
            password = simpledialog.askstring(
                "Create Account", "Enter your password:", show='*')
...
def initialize_main_tab(self):
        ...
        self.chat_area.pack(side=tk.LEFT, padx=10, pady=10,
        ...
        self.send_button = tk.Button(
            self.main_tab, text="Send", command=self.send_message)
        self.send_button.pack(padx=10, pady=10)

        self.solo_game_button = tk.Button(
            self.main_tab, text="Start Solo Game", command=self.start_solo_game)
        self.solo_game_button.pack(padx=10, pady=10)

        self.status_label = tk.Label(
            self.main_tab, text="Connecting...", fg="blue")
...
def start_solo_game(self):
        ...
        if os.path.exists('selected_cards.json'):
            ...
            # Ouvrir le jeu Triple Triad contre l'IA dans une fenêtre Pygame
            game = TripleTriadGame(player_cards, ai_player.hand)
            game.run()
        else:
    ...
    def on_part(self, connection, event):
        ...
        if event.source.nick in our users:
    ...
    def accept_defy(self, opponent, opponent_cards):
        ...
        if os.path.exists('selected_cards.json'):
            ...
            # Ouvrir le jeu Triple Triad contre l'adversaire dans une fenêtre Pygame
