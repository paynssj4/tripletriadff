...
from card import Card  # Assurez-vous d'importer la classe Card correctement
...
class TripleTriadGame(QWidget):
    ...
    def load_images(self):
        self.card_back_blue = QPixmap("/media/greg/FE1812A418125BC9/test jeux tt/Nouveau dossier/Img/cardbleu.png").scaled(100, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        self.card_back_red = QPixmap("/media/greg/FE1812A418125BC9/test jeux tt/Nouveau dossier/Img/cardrouge.png").scaled(100, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        self.board_image = QPixmap("/media/greg/FE1812A418125BC9/test jeux tt/Nouveau dossier/Img/board-mat.jpg").scaled(1258, 734, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def display_player_cards(self):
        ...
        for card in self.player_cards:
            try:
                pil_image = card.get_image().resize((100, 100), Image.Resampling.LANCZOS)
                qt_image = ImageQt.ImageQt(pil_image)  # Convertir l'image PIL en QImage
                img = QPixmap.fromImage(qt_image).scaled(100, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation)
