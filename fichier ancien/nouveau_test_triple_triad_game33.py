...
def dessiner_cartes(fenetre, cartes_joueur, cartes_ia, start_time, selected_card_index=None):
    ...
    for i, card in enumerate(cartes_joueur):
        if elapsed_time > i * delay:
            ...
            card.x = 50
            card.y = y_pos
            #filtrage de la carte pour la cacher
            if selected_card_index is not None and i != selected_card_index :
                fenetre.blit(card.background, (50, y_pos))
                fenetre.blit(card.image, (50, y_pos))
...
def dessiner_jeu(fenetre, game_logic, selected_card=None, mouse_pos=None, start_time=None):
    ...
    dessiner_cartes(fenetre, game_logic.player_cards, game_logic.ai_player.hand, start_time, game_logic.selected_card_index)
...
def main():
    ...
    while running and not game_logic.check_game_over():
                    ...
                    for cell_name, coords in cell_coords.items():
                        ...
                        if cell_rect.collidepoint(x, y):
                            if game_logic.selected_card_index is not None and len(player_cards) > game_logic.selected_card_index and game_logic.selected_card == player_cards[game_logic.selected_card_index]:
                                ...
                                player_cards.pop(game_logic.selected_card_index)
        ...
        dessiner_jeu(fenetre, game_logic, game_logic.selected_card, mouse_pos, start_time)
